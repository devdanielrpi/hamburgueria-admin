...
export default function Pedidos() {
  ...
  function toggleAdicional(itemId, ad) {
    ...
    setItens(prev =>
      prev.map(i => {
        ...
        const exists =
          i.adicionais.some(a => a.id === ad.id)
  ...
  return (
    <div className="p-6 bg-zinc-100 min-h-screen">
      ...
      {/* FILTROS */}
      <div className="bg-white p-4 rounded-2xl mb-5 flex flex-wrap gap-3">
            ...
            <div className="mb-3">
              ...
              {produtos.map(p => (
                ...
                <button
                  key={p.id}
                  onClick={() => addItem(p)}
                  className="block w-full text-left border p-2 mb-2 rounded-xl"
                >
                  {p.nome} - R$ {p.preco}
                </button>
